...
import square_matrix.service.FileOpener;
import square_matrix.service.Solver;
...
public class MainForm {
    ...
    private JTextField rowsAndColsTextField;
    ...
    private void initialize() {
        ...
        JLabel label = new JLabel("Rows and columns number");
        label.setBounds(615, 11, 216, 14);
        frame.getContentPane().add(label);

        rowsAndColsTextField = new JTextField("3");
        rowsAndColsTextField.setBounds(615, 34, 161, 20);
        frame.getContentPane().add(rowsAndColsTextField);
        rowsAndColsTextField.setColumns(10);

        JButton btnNewButton = new JButton("Create table");
        btnNewButton.addActionListener(e -> createTable(-1));
    ...
    private void saveFile() {
        try {
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                fw.append(String.valueOf(matrixTable.getRowCount()));
                fw.append("\r\n");
    ...
    private void createTable(int rowsAndCols) {
        matrixTable.removeAll();
        answersTable.removeAll();
        vectorTable.removeAll();
        if (rowsAndCols < 0) {
            rowsAndCols = Integer.parseInt(rowsAndColsTextField.getText());
        }
        matrixTable.setModel(matrixTableModel);
        matrixTableModel.setColumnCount(rowsAndCols);
        matrixTableModel.setRowCount(rowsAndCols);
        matrixTable.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
        matrixTable.setRowHeight(308 / rowsAndCols);

        answersTable.setModel(answerTableModel);
        answerTableModel.setColumnCount(rowsAndCols);
        ...
        vectorTableModel.setRowCount(rowsAndCols);
        vectorTableModel.setColumnCount(1);
        vectorTable.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
        vectorTable.setRowHeight(308 / rowsAndCols);
    }

    private void solve() {
        try {
            double[][] matrixData = readMatrix();
            if(!Solver.isMatrixSymmetric(matrixData)){
                JOptionPane.showMessageDialog(frame, "The matrix is not symmetric");
                return;
            }
            double[] vector = readVector();
